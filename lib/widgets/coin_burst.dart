...
class _CoinBurstWidgetState extends State<CoinBurstWidget> with SingleTickerProviderStateMixin {
  ...
  void _generateParticles() {
    ...
    // Generate 25 coins
    for (int i = 0; i < 25; i++) {
      final angle = -math.pi / 2 + (_rnd.nextDouble() - 0.5) * math.pi; // Upwards cone
  ...
  Widget build(BuildContext context) {
    ...
    return CustomPaint(
      painter: CoinBurstPainter(particles: _particles, progress: _controller.value),
      size: Size.infinite,
    );
  }
}
...
class CoinBurstPainter extends CustomPainter {
  ...
  void _draw3DCoin(Canvas canvas, double rx, double ry, double rz, double opacity) {
    ...
    for (int i = 0; i < 6; i++) {
      ...
      baseVerts.add(MathVector3(math.cos(angle) * radius, math.sin(angle) * radius, 0));
    }
    
    final holeRadius = radius * 0.35;
    for (int i = 0; i < 4; i++) {
      double angle = (i * math.pi / 2) + (math.pi / 4);
      holeVerts.add(MathVector3(math.cos(angle) * holeRadius, math.sin(angle) * holeRadius, 0));
    }
    ...
    List<MathVector3> topOuter = baseVerts.map((v) => rotate(MathVector3(v.x, v.y, thickness / 2))).toList();
    List<MathVector3> topInner = holeVerts.map((v) => rotate(MathVector3(v.x, v.y, thickness / 2))).toList();
    
    List<MathVector3> botOuter = baseVerts.map((v) => rotate(MathVector3(v.x, v.y, -thickness / 2))).toList();
    List<MathVector3> botInner = holeVerts.map((v) => rotate(MathVector3(v.x, v.y, -thickness / 2))).toList();

    // Determine normal of top face to see if it's facing us
    // Normal = cross product of two edge vectors
    MathVector3 edge1 = MathVector3(topOuter[1].x - topOuter[0].x, topOuter[1].y - topOuter[0].y, topOuter[1].z - topOuter[0].z);
    MathVector3 edge2 = MathVector3(topOuter[2].x - topOuter[1].x, topOuter[2].y - topOuter[1].y, topOuter[2].z - topOuter[1].z);
    double normalZ = edge1.x * edge2.y - edge1.y * edge2.x;

    bool topFacing = normalZ > 0;

    // Helper to draw a quad (for sides)
    void drawQuad(MathVector3 p1, MathVector3 p2, MathVector3 p3, MathVector3 p4, Color c) {
      ...
      canvas.drawPath(path, Paint()..color = goldDark..style = PaintingStyle.stroke..strokeWidth = 0.5);
    }

    // Helper to draw face
    void drawFace(List<MathVector3> outer, List<MathVector3> inner, Color fill, Color highlight) {
      final path = Path();
      ...
      canvas.drawPath(path, Paint()..color = highlight..style = PaintingStyle.stroke..strokeWidth = 1.0);
      canvas.drawPath(innerPath, Paint()..color = goldDark..style = PaintingStyle.stroke..strokeWidth = 1.0);
      ...
      for(int i=0; i<outer.length; i++){
         ...
         if(i==0) embossPath.moveTo(px, py);
         else embossPath.lineTo(px, py);
      }
      embossPath.close();
      canvas.drawPath(embossPath, Paint()..color = highlight..style = PaintingStyle.stroke..strokeWidth = 0.8);
    }
    ...
    if (topFacing) {
      drawFace(botOuter, botInner, goldSide, goldSide); // Draw bot face just in case

...
export interface FraudAlertEntity {
  id: string;
  userId: string;
  userEmail: string;
  alertType: 'emulator_detected' | 'score_velocity_spike' | 'vpn_proxy_detected' | 'multi_account_farming';
...
export interface SupportTicketEntity {
  ...
  email: string;
  subject: string;
  category: 'redemption' | 'game_issue' | 'account' | 'coins';
  message: string;
...
class InMemoryDatabase {
  ...
  public redemptions: Map<string, RedemptionEntity> = new Map();
  public fraudAlerts: Map<string, FraudAlertEntity> = new Map();
  public systemConfigs: Map<string, SystemConfigEntity> = new Map();
  public deletionRequests: Map<string, DeletionRequestEntity> = new Map();
  public supportTickets: Map<string, SupportTicketEntity> = new Map();
  public auditLogs: AuditLogEntity[] = [];
  public redisCache: Map<string, { value: string; expiresAt?: number }> = new Map();
  ...
  public seedDefaults(): void {
    ...
    const d1: DeletionRequestEntity = {
  ...
  public purgeRedisCache(pattern: string): number {
    let count = 0;
